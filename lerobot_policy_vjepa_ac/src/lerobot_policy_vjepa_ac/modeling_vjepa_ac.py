...
class VjepaAcPolicy(PreTrainedPolicy):
    ...
    def __init__(self, config: VjepaAcConfig, dataset_stats=None, **kwargs):
        ...
        with torch.device(device):
            encoder_output = torch.hub.load(
                config.encoder_repo_id,
                config.model_name
            )
    ...
    def select_action(self, batch: dict[str, torch.Tensor], **kwargs) -> torch.Tensor:
        ...
        with torch.no_grad():
            if images.ndim == 5:
                # Use the last frame for MPC sequence start
                img_seq = images[:, :, -1:]
            else:
        ...
        for b in range(B):
            ...
            for _ in range(self.config.cem_num_iters):
                ...
                costs = torch.mean(torch.abs(final_latent - g_lat) ** getattr(self.config, "loss_exp", 1.0), dim=(1,2))
    ...
    def forward(
        ...
        with torch.no_grad():
            if images.ndim not in (4, 5):
                raise ValueError(f"images tensor for key '{image_key}' has shape {images.shape}, expected 4D or 5D")
            
            if images.ndim == 4:
                # Single-frame case: [B, C, H, W] -> [B, 1, N_patches, D]
                images = images.unsqueeze(2)
                z = self.encoder(images)
                target_latents = z.unsqueeze(1)  # [B, 1, N, D]
            else:
                # Multi-frame case: [B, C, T, H, W]
        ...
        z_tf = _step_predictor(z_ctxt, actions, states[:, :-1], extrinsics)
